<?php
/*
 *  Class to integrate with Facebook's API.
...
class EpiFacebook
{
  ...
  protected $requestTokenUrl= 'https://graph.facebook.com/oauth/authorize';
  protected $accessTokenUrl = 'https://graph.facebook.com/oauth/access_token';
  protected $authorizeUrl   = 'https://graph.facebook.com/oauth/authorize';
  protected $apiUrl         = 'https://graph.facebook.com';
  protected $userAgent      = 'EpiFacebook';
  ...
  {     
    $params = array('client_id' => $this->clientId, 'client_secret' => $this->clientSecret, 'redirect_uri' => $redirectUri, 'code' => $code);
    $qs = http_build_query($params);
	return $this->request('GET', "{$this->accessTokenUrl}", $params);
  }

  public function getAuthorizeUrl($redirectUri,$scope)
  {
    $params = array('client_id' => $this->clientId, 'scope' => implode(",",$scope), 'redirect_uri' => $redirectUri);
    $qs = http_build_query($params);
  ...
  private function request($method, $endpoint, $params = null)
  {
    ...
    if($method === 'GET')
   	{
    	if ($this->accessToken) {
     	if (count($params)) {
      	    	$url .= "?access_token=".$this->accessToken."&".http_build_query($params, '', '&');
	    	} else {
    ...
    $resp = new EpiFacebookJson(EpiCurl::getInstance()->addCurl($ch), $this->debug);
...
class EpiFacebookJson implements ArrayAccess, Countable, IteratorAggregate
{
  private $debug;
  private $__resp;
  ...
  public function __get($name)
  {
    ...
    elseif(($this->code < 200 || $this->code >= 400) && !isset($accessible[$name]))
      EpiFacebookException::raise($this->__resp, $this->debug);
...
class EpiFacebookException extends Exception 
{
  public static function raise($response, $debug)
  {
    ...
    {
      case 400:
        throw new EpiFacebookBadRequestException($message, $response->code);
      case 401:
        throw new EpiFacebookNotAuthorizedException($message, $response->code);
      case 403:
        throw new EpiFacebookForbiddenException($message, $response->code);
      case 404:
        throw new EpiFacebookNotFoundException($message, $response->code);
      default:
        throw new EpiFacebookException($message, $response->code);
    }
  }
}
class EpiFacebookBadRequestException extends EpiFacebookException{}
class EpiFacebookNotAuthorizedException extends EpiFacebookException{}
class EpiFacebookForbiddenException extends EpiFacebookException{}
class EpiFacebookNotFoundException extends EpiFacebookException{}